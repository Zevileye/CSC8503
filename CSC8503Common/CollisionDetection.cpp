...
static std::map<int, int> ignoreLayerMap;

void AddIgnoreLayer(int layer1, int layer2)
{
	ignoreLayerMap.insert(std::make_pair(layer1, layer2));
	ignoreLayerMap.insert(std::make_pair(layer2, layer1));
}
...
bool CollisionDetection::ObjectIntersection(GameObject* a, GameObject* b, CollisionInfo& collisionInfo) {
	...
	auto found = ignoreLayerMap.find(a->GetLayer());
	if (found != ignoreLayerMap.end())
	{
		if (found->second == b->GetLayer())
		{
			return false;
		}
	}
...
bool CollisionDetection::SphereIntersection(const SphereVolume& volumeA, const Transform& worldTransformA,
	...
	float penetration = delta.Length() - (volumeA.GetRadius() + volumeB.GetRadius());
	if (penetration < 0)
	{
		collisionInfo.AddContactPoint(worldTransformA.GetPosition() + delta.Normalised() * (volumeA.GetRadius() + penetration)
			, worldTransformB.GetPosition() - delta.Normalised() * (volumeB.GetRadius() + penetration)
			, delta.Normalised(), -penetration);
		return true;
	}
...
bool NCL::CollisionDetection::OBBSphereIntersection(const OBBVolume& volumeA, const Transform& worldTransformA, const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	Vector3 positionA(0, 0, 0);

	Transform newTransform;
	newTransform.SetPosition(worldTransformA.GetPosition());
	newTransform.SetOrientation(worldTransformA.GetOrientation());
	Vector3 positionB = newTransform.GetMatrix().Inverse() * Vector4(worldTransformB.GetPosition(), 1.0);
	...
	if (distance < volumeB.GetRadius()) {// yes , we ¡¯re colliding !
		...
		Vector3 localB = positionB - delta.Normalised() * volumeB.GetRadius();

		Vector3 normal = (newTransform.GetMatrix() * Vector4(collisionNormal, 0.0)).Normalised();
		collisionInfo.AddContactPoint(newTransform.GetMatrix() * localA, newTransform.GetMatrix()  * localB,
			normal, penetration);
		return true;
	}
	return false;
